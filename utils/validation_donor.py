...
def validate_order_data(attrs):
    ...
    if "image" in attrs and len(attrs.get("image")) < 0 and "image_path" in attrs and attrs.get("image_path",
                                                                                                None) is not None:
        if "image_path" in attrs and len(attrs.get("image_path")) < 1:
            return "Please provide a valid image path"
        if "image_quantity" in attrs and attrs.get("image_quantity", None) is None and attrs.get("image_quantity",
                                                                                                 None) is not Number:
